...
export class EmailService {
  /**
   * Generates a sleek, professional responsive HTML template for 12 PM absence alerts
   */
  static get12pmAbsenceHtml(payload: EmailPayload): string {
    return `
<!DOCTYPE html>
<html>
<head>
...
<body>
  <div class="container">
    ...
    <div class="content">
      <p>Dear <strong>${payload.trainerName}</strong>,</p>
      ...
      <div class="badge-box">
        ...
        <div class="badge-desc">
          Our attendance monitoring engine recorded that you have not logged in to the Faculty Portal by the standard 12:00 PM cutoff today (${payload.date}), and no prior approved leave was registered in the system.
        </div>
      </div>
      ...
      <table class="details-table">
        <tr>
          ...
          <td class="val">${payload.trainerName}</td>
        </tr>
        <tr>
          <td class="label">Date:</td>
          <td class="val">${payload.date}</td>
        </tr>
        <tr>
          <td class="label">Cutoff Evaluated:</td>
          <td class="val">${payload.cutoffTime || '12:00 PM (IST)'}</td>
        </tr>
        <tr>
...
`;
  ...
  /**
   * Native TLS SMTP sender for Gmail
   */
  static async send12pmAbsenceEmail(payload: EmailPayload): Promise<{ success: boolean; message: string }> {
    const user = process.env.SMTP_USER || 'kanzariyapratik124@gmail.com';
    const pass = process.env.SMTP_PASS || 'xrdtkwzkayzeerkf';
    const html = this.get12pmAbsenceHtml(payload);
      ...
      try {
        ...
        socket.on('data', (data) => {
          ...
          if (step === 0 && msg.startsWith('220')) {
            ...
            socket.write('QUIT\r\n');
            resolve({ success: true, message: `Email delivered to ${payload.to}` });
